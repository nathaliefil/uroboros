...
namespace Uroboros.syntax.interpretation.functions
{
    ...
    {
        public static INumerable Build(Token nameToken, List<Token> tokens)
        {
            string name = nameToken.GetContent().ToLower();
            List<Argument> args = ArgumentsExtractor.GetArguments(tokens);
